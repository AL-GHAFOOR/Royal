...
{
    public partial class NICUBill : Form
    {
        ...
        {
            LoadConsoultService(cmbPid.Text, "NICU");
            LoadPatientService(cmbPid.Text, "NICU");
            LoadPatientPathologyService(cmbPid.Text, "NICU");
        }
        ...
        {
            DataTable AllDoctor = new ServiceManager().GetPatientService("NICU");
        ...
        private void IPBillingSetup_Load(object sender, EventArgs e)
        {
            ...
            //-----for patientService---//_session.ChkPermission(MainWindow.userName);
        ...
        public void PathologyMaster()
        {
            ...
            repositoryItemLookUpEdit5.DataSource = PMaster;
            repositoryItemLookUpEdit5.DisplayMember = "Alias";
            repositoryItemLookUpEdit5.ValueMember = "Alias";
        ...
        private void gridView_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                if (e.Column.FieldName == "ConQty")
                {
                    ...
                    double qty = Convert.ToDouble(gridView.GetRowCellValue(e.RowHandle, "ConQty"));
                    double confee = Convert.ToDouble(gridView.GetRowCellValue(e.RowHandle, "ConFee"));

                    double totalFee = qty * confee;
                    gridView.SetRowCellValue(e.RowHandle, "TotalConsFee", totalFee.ToString("0.00"));
                }
                else if (e.Column.FieldName == "ConFee")
                {
                    ...
                    double qty = Convert.ToDouble(gridView.GetRowCellValue(e.RowHandle, "ConQty"));
                    double totalFee = qty* confee;
                    gridView.SetRowCellValue(gridView.FocusedRowHandle, "TotalConsFee", totalFee.ToString("0.00"));
        ...
        public void CellvaluechnagedForPath_Service(string value)
        {
            try
            {
                ...
                gridView1.SetFocusedRowCellValue("serviceDate", dtpPatientService.Value);
            }
            catch (Exception)
            {
        ...
        {
            NicuAddmission setup = new NICUBillManager().GetNICUPatientInfo(pid);
            ...
            try
            {
                ...
                //Consult---
                   // LoadDataForIPD(cmbPid.Text);
                    LoadConsoultService(selectId,"NICU");
                    LoadPatientService(selectId, "NICU");
                    LoadPatientPathologyService(cmbPid.Text, "NICU");
        ...
        private void btnConsultBill_Click(object sender, EventArgs e)
        {
            ...
            aService.AConsultBill = new ServiceManager().GetConsultBillServicesByPatientId(cmbPid.Text, "NICU", "Consult", dtpConsultBill.Value);
        ...
        private void btnSavePSBill_Click(object sender, EventArgs e)
        {
            ...
            saveService.GetPatientServices = new ServiceManager().GetPatientServiceByPatientId(cmbPid.Text, "NICU", "",dtpConsultBill.Value);
        ...
        private void repositoryItemLookUpEdit4_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (edit != null)
            {
                ...
                DataRow row = Pathology.NewRow();
                int rowIndex = gridView1.RowCount - 1;
        ...
        {
            Service saveService = new Service();
                saveService.GetPathologyServices = new ServiceManager().GetPathologyServiceByPatientId(cmbPid.Text, "NICU", "", dtpConsultBill.Value);
            saveService.OPID = cmbPid.Text;
        ...
        {
            gridView.DeleteRow(gridViewServiceBill.GetFocusedDataSourceRowIndex());
        }
